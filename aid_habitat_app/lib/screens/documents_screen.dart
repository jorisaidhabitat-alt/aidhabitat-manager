...
class _DocumentsScreenState extends State<DocumentsScreen>
  ...
  /// Diagnostique pourquoi un aperçu de document a échoué et affiche un
  /// message ciblé avec un bouton « Réessayer » qui force un pull remote
  /// avant de réouvrir le doc. Couvre :
  ///   • URL absente (doc record sans `remote_public_url`)
  ///   • 404 (doc supprimé côté serveur ou contenu vide / corrompu)
  ///   • 401/403 (token expiré → l'AuthService doit se ré-authentifier)
  ///   • erreur réseau (offline, serveur en panne)
  ///   • réponse HTML (erreur de routing Vercel)
  Future<void> _handlePreviewFailure(DocItem doc) async {
    String message;
    ...
    if (url.isEmpty) {
      message =
          'Ce document n\'a pas d\'URL distante. Synchronisez à nouveau.';
    } else {
      final diag = await MediaCacheService.instance.diagnose(
        url,
        headers: {'X-App-Session': AppConfig.appSessionToken},
      );
      if (diag == null) {
        message = 'Aperçu indisponible (raison inconnue).';
      } else if (diag.isAuthError) {
        message =
            'Session expirée — reconnectez-vous puis réessayez.';
        offerRetry = false;
      } else if (diag.isMissing) {
        message =
            'Document introuvable ou corrompu côté serveur. '
            'Supprimez-le puis réimportez-le.';
      } else if (diag.isServerError) {
        message = 'Erreur serveur (HTTP ${diag.statusCode}).';
      } else if (diag.isNetworkError) {
        message = 'Pas de réseau. Réessayez quand vous serez connecté.';
      } else if (diag.isEmptyBody) {
        message =
            'Le serveur a renvoyé un fichier vide (corrompu côté serveur).';
      } else {
        message =
            'Aperçu indisponible (HTTP ${diag.statusCode}).';
      }
    }
      ...
      SnackBar(
        ...
        action: offerRetry
            ? SnackBarAction(
                label: 'Réessayer',
                onPressed: () async {
                  // Force un pull frais des docs (le `remote_public_url`
                  // peut avoir changé entre-temps), puis ré-ouvre.
                  await _dataService.refreshDocumentsFromRemote(
                    widget.dossier.patient.id,
                  );
                  await _loadDocuments(silent: true);
                  if (!mounted) return;
                  // Retrouve le doc fraîchement chargé (l'instance a pu
                  // changer si remote_public_url a été mis à jour).
                  final fresh = _documents.firstWhere(
                    (d) => d.id == doc.id,
                    orElse: () => doc,
                  );
                  await _previewDocument(fresh);
                },
              )
            : null,
      ),
  ...
  Future<void> _showWebImagePreview(DocItem doc) async {
    ...
    if (bytes == null || !mounted) {
      // Diagnostic réseau : on demande à `MediaCacheService` ce qui s'est
      // vraiment passé pour pouvoir afficher un message clair. Avant ce
      // diagnostic l'utilisateur voyait "Aperçu indisponible" sans
      // distinguer 404 (fichier corrompu côté serveur), 401 (token
      // expiré), erreur réseau, etc.
      await _handlePreviewFailure(doc);
      return;
    }
